...
public class PicoVREye : MonoBehaviour
{
    ...
    public void AyscRender()
    {
        ...
        {
            transform.GetComponent<Camera>().targetTexture = PicoVRManager.SDK.currentDevice.eyeTextures[0];
        }
        else
        {
            transform.GetComponent<Camera>().targetTexture = PicoVRManager.SDK.currentDevice.eyeTextures[1]; 
        }
#endif
    }
    ...
    void OnPreCull()
    {
        if (!PicoVRManager.SDK.currentDevice.Async)
        {
            ...
            //Debug.Log("kkk OnPreCull " + (int)eye + " depth : " + camera.depth);
        }
    }
