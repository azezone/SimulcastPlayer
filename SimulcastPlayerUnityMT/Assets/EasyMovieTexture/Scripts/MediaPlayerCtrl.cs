...
public class MediaPlayerCtrl : MonoBehaviour {
	...
	void Update()
	{
		...
		if(m_CurrentState == MEDIAPLAYER_STATE.PLAYING)
		{
			if(m_bCheckFBO == false)
			{
				...
				if(m_bSupportRockchip)
				{
					m_VideoTexture = new Texture2D(Call_GetVideoWidth(),Call_GetVideoHeight(),TextureFormat.RGB565,true);
					
				}
				else
				{
                    m_VideoTexture = new Texture2D(Call_GetVideoWidth(),Call_GetVideoHeight(),TextureFormat.RGBA32, true);
				}
	...
	public void Load(string strFileName)
	{
		...
		{
			Call_SetRockchip(m_bSupportRockchip);
            Debug.LogError("*********m_bSupportRockchip:*********" + strFileName);
            if (strFileName.Contains("://"))
			{
                Debug.LogError("*********Call_Load:*********"+strFileName);
				Call_Load(strFileName,0);
			}
			else
			{
                Debug.LogError("*********CopyStreamingAssetVideoAndLoad*********");
	...
	IEnumerator CopyStreamingAssetVideoAndLoad(string strURL)
	{
        ...
        Debug.LogError("*********CopyStreamingAssetVideoAndLoad********* path:"+ write_path);
